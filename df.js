...
function Students() {
  ...
  const handleDelete = async (id) => {
    if (window.confirm("This will permanently remove the student from the database. You can't undo this! Click OK to proceed.")) {
  ...
  const redactPhoneNumber = (phoneNumber) => {
    if (!phoneNumber) {
      return ''; // Return an empty string if phone number is not available
    }

    return phoneNumber.replace(/\d/g, 'X'); // Replace digits with 'X'
  };
  ...
  return (
    <>
      ...
      <div>
        <h4>Showing {students.length} students</h4>
      </div>
      <div className="studentPageDiv">
        {students.map((student) => (
          <div className="student" key={student._id}>
            <div id="student-avatar">
              <h4>{student.firstName[0].toUpperCase()}{student.middleName[0].toUpperCase()}{student.lastName[0].toUpperCase()}</h4>
            </div>
            <div className="studentInfo">
              ...
              <p><strong>Phone No:</strong> {redactPhoneNumber(student.phoneNumber)}</p>
